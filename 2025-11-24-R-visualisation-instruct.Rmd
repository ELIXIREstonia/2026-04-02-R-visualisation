...
<https://github.com/ELIXIREstonia/2025-11-24-R-visualisation>
...
:   An experiment on the effects of anti-anxiety medicine on memory recall when
    being primed with happy or sad memories. The experiment was conducted on novel
    Islanders who mimic real-life humans in response to external factors.
...
Isd <- Isd %>%
  mutate(
    age_group = case_when(
      ...
      age > 50 ~ "Senior",
      TRUE ~ "Unknown" # Good practice to have a fallback
    )
  )

# Check the results
head(Isd %>% select(age, age_group))
table(Isd$age_group)
Isd
```
...
A bar plot displays the frequency or count of categorical data. 
...
```{r}
# Create a histogram of Islander dataset age variable.
...
```{r}
# Create a box plot of Diff by Drug for the Islander dataset
...
Feel free to go to 
[Geoms list](https://ggplot2.tidyverse.org/reference/index.html#geoms)
and explore more geoms.
...
to see the distribution of observations within each `Drug` category.
...
# Violin plot of Diff by Drug
violin_plot <- ggplot(Isd, aes(x = Drug, y = Diff, fill = Drug)) +
  geom_violin(trim = FALSE, alpha = 0.6) + # Violin plot to show density/distribution
  geom_point(position = position_jitter(width = 0.2), alpha = 0.3, size = 1.5) + # Jittered points to show individual observations
  labs(
    title = "Distribution of Memory Score Differences by Drug",
...
## Scaling Numeric (continious) and Discrete Variables
...
Example: Log Transformation of Horsepower   
Let's create a scatter plot of miles
per gallon (mpg) vs. horsepower (hp) and scale the `y` axis using log
transformations:


```{r}
# Scatter plot with non-scaled y-axis using mtcars data
...
```
```{r}
# Scatter plot with log2-scaled y-axis using mtcars data
scatter_plot_scaled <- ggplot(mtcars, aes(x = mpg, y = hp)) +
  geom_point(alpha = 0.5) +
...
distinct values. In ggplot2, you can refactor factor variables to
enhance your visualizations.

### Refactoring

Refactoring involves changing the levels of a factor variable. You can use the
base R `factor()` function to specify the levels and their corresponding labels.

```{r}
# Recode the Drug variable using base R
Isd_refactored <- Isd %>%
  mutate(Drug_name = factor(Drug,
                            levels = c("A", "T", "S"),
                            labels = c("Alprazolam", "Triazolam", "Sugar")))

head(Isd_refactored)
```

In this example, we refactored the `Drug` variable by changing the levels from
letters to actual drug names using the `labels` argument in `factor()`.

Now let's create a bar plot of the refactored Drug_name variable:

```{r}
# Create a bar plot of the refactored Drug_name variable
bar_plot_refactored <- ggplot(Isd_refactored, aes(x = Drug_name, fill = Happy_Sad_group)) +
  geom_bar(position = "dodge") +
  labs(x = "Drug (refactored)", fill = "Happy/Sad Group") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

bar_plot_refactored
```

In this plot, we used the refactored `Drug_name` variable to create a
more informative visualization. By refactoring factor variables,
...
```{r}
# Custom visualization: Memory Score Difference by Dosage and Drug
# We'll assume 'Dosage' is a variable in the dataset based on the description, 
# but looking at the file, it seems dosage info might be encoded in Drug or separate.
# Let's check the columns first. If Dosage isn't explicit, we'll stick to Drug and Happy_Sad_group.

# Let's create a comprehensive plot showing Diff by Drug and Happy_Sad_group
custom_plot <- ggplot(Isd, aes(x = Drug, y = Diff, fill = Happy_Sad_group)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.7) +
  geom_point(position = position_jitterdodge(jitter.width = 0.2), alpha = 0.4, size = 1) +
  facet_wrap(~age_group) + # Facet by the new age_group variable we created
  labs(
    title = "Memory Score Difference by Drug and Emotional Priming",
    subtitle = "Faceted by Age Group",
    x = "Drug Treatment",
    y = "Difference in Memory Score",
    fill = "Priming Group"
  ) +
...
Before you leave, please provide
['feedback'](https://forms.gle/1PfHcBkVgktUG7zU6)!
